...
function getApiBaseUrl() {
  ...
  if (hostname === 'localhost' || hostname === '127.0.0.1') {
    ...
    return '/api';
  } else if (hostname === 'shivamsoni.duckdns.org') {
    // Production domain
    console.log('🔧 Production domain detected, returning:', `${protocol}//${hostname}:8443/api`);
    return `${protocol}//${hostname}:8443/api`;
  } else if (hostname.includes('shivamsoni.duckdns.org') || hostname.includes('app.shivamsoni.duckdns.org')) {
    // Tunnel subdomain or any shivamsoni.duckdns.org subdomain - use relative path
